...
function initHeader() {
    ...
    const menuTl = gsap.timeline({ paused: true, onReverseComplete: () => gsap.set(mobileMenu, { display: 'none' }) });
    menuTl.fromTo(mobileMenu, { clipPath: 'inset(0 0 100% 0)' }, { clipPath: 'inset(0 0 0% 0)', display: 'flex', duration: 0.4, ease: 'power2.inOut' })
          .from(mobileMenu.querySelectorAll('.mobile-link'), { y: 20, opacity: 0, duration: 0.3, stagger: 0.05, ease: 'power2.out' }, "-=0.2");
    const toggleMenu = () => {
        isOpen = !isOpen;
...
function initPartnership() {
    ...
    gsap.set(partnershipSection.querySelectorAll('.partnership-title, .partnership-item, .animation-container, .feature-visual-group, .backdrop-panel, .feature-details'), {
        clearProps: "all"
    });
    ...
    if (window.innerWidth >= 1024 && !PREFERS_REDUCED_MOTION) {
        ...
        if (triggers.length > 0) {
            ...
            const setActive = (index) => {
                ...
                gsap.to(indicator, { y: triggers[index].offsetTop, duration: 0.4, ease: 'power3.out' });
                triggers.forEach((t, i) => gsap.to(t, { opacity: i === index ? 1 : 0.6, duration: 0.3 }));

                const tl = gsap.timeline({ onComplete: () => isAnimating = false });
                tl
                  .to(visualGroups, { opacity: 0, duration: 0.2, ease: 'power1.in', display: 'none' })
                  ...
                  .fromTo(visualGroups[activeIndex], 
                      { y: 50, opacity: 0 }, 
                      { y: 0, opacity: 1, duration: 0.8, ease: 'power2.out', display: 'flex' }
                  )
                  .to(backdropPanel, { y: 0, duration: 0.5, ease: 'power3.out' }, "-=0.5")
                  .to(detailsTexts[activeIndex], { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.6, ease: 'power3.out' }, "-=0.3");
            };
            ...
            const initialTl = gsap.timeline({
                scrollTrigger: { trigger: partnershipSection, start: 'top 70%', once: true, onEnter: () => isAnimating = true },
                onComplete: () => isAnimating = false
            });
            initialTl
                .from(partnershipSection.querySelectorAll('.partnership-title, .partnership-item, .animation-container'), { opacity: 0, y: 20, stagger: 0.1, duration: 0.6 })
                
                // --- KLUCZOWA POPRAWKA (dla animacji wejściowej) ---
                .fromTo(visualGroups[0], 
                    { y: 50, opacity: 0 }, 
                    { y: 0, opacity: 1, duration: 0.8, ease: 'power2.out', display: 'flex' }
                )
                .to(backdropPanel, { y: 0, duration: 0.5, ease: 'power3.out' }, "-=0.5")
                .to(detailsTexts[0], { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.6, ease: 'power3.out' }, "-=0.3");
        }
    } else { // Twoja działająca logika dla mobile
        if (!PREFERS_REDUCED_MOTION) {
                    ...
                    if (!isOpen) {
                        accordionItems.forEach(otherItem => {
                            if (otherItem.classList.contains('open')) {
                                gsap.to(otherItem.querySelector('.accordion-content'), { height: 0, duration: 0.4 });
...
function initProcess() {
    ...
    if (!PREFERS_REDUCED_MOTION) {
        gsap.set('.process-title, .process-step-trigger, .process-content-wrapper', { opacity: 0, y: 30 });
        gsap.timeline({
            scrollTrigger: { trigger: processSection, start: 'top 80%', once: true }
        })
        .to('.process-title', { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' })
        .to('.process-step-trigger', { opacity: 1, y: 0, duration: 0.5, stagger: 0.1, ease: 'power2.out' }, '-=0.4')
        .to('.process-content-wrapper', { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, "-=0.5");
    } else {
        gsap.set('.process-title, .process-step-trigger, .process-content-wrapper', { opacity: 1, y: 0 });
    }
    ...
    if (triggers.length > 0 && contents.length > 0 && indicator) {
        ...
        const setActiveItem = (index) => {
            gsap.to(indicator, { y: triggers[index].offsetTop + 16, duration: 0.4, ease: 'power3.out' });
            ...
            gsap.fromTo(contents[index], { opacity: 0, y: -15 }, { opacity: 1, y: 0, duration: 0.3, ease: 'power1.out', delay: 0.2 });
        };
        ...
        gsap.set(triggers.slice(1).flatMap(t => [t.querySelector('.step-number'), t.querySelector('.step-title')]), { color: '#A1A1A1' });
        ...
        processAccordionItems.forEach(item => {
            ...
            if (header && !header.dataset.listenerAttached) {
                header.addEventListener('click', () => {
                    ...
                    if (!isOpen) {
                        processAccordionItems.forEach(otherItem => {
                             if (otherItem !== item) {
                                ...
                                const otherIcon = otherItem.querySelector('.process-accordion-icon');
                    ...
                    gsap.to(content, { height: isOpen ? 0 : 'auto', duration: 0.4, ease: 'power2.inOut' });
...
function initMetrics() {
    ...
    const runAnimations = () => {
        if (!PREFERS_REDUCED_MOTION) {
            ...
            gsap.set('.metrics-title, .metrics-subtitle, .metric-card', { opacity: 1, y: 0, scale: 1 });
        }
                ...
                if (fcpElement) {
                     try {
                        new PerformanceObserver((entryList) => {
                            ...
                            gsap.set(fcpBorder, { strokeDasharray: fcpPerimeter, strokeDashoffset: fcpPerimeter });
                        
                            const fcpTl = gsap.timeline();
                            fcpTl.to(counter, {
                                val: fcpTimeInSeconds, duration: 1.5, ease: 'power2.out',
                                onUpdate: () => { fcpElement.textContent = counter.val.toFixed(2); }
                            }).to(fcpBorder, { strokeDashoffset: 0, duration: 1.5, ease: 'none' }, 0);
...
function initTechnologies() {
    ...
    gsap.set([mainTitle, mainSubtitle, subtitle1, subtitle2, coreTechGrid.children, frameworksGrid.children], { opacity: 0, y: 20 });

    // Dodajemy animacje w prawidłowej sekwencji
    tl
        .to(mainTitle, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' })
        ...
        .to(frameworksGrid.children, { opacity: 1, y: 0, stagger: 0.05, ease: 'power1.out' }, "-=0.3");
}
...
function initUslugiPage() {
    ...
    if (!PREFERS_REDUCED_MOTION) {
        gsap.set("#page-hero-title, #page-hero-subtitle, #uslugi .grid > *, #faq h2, #faq .space-y-8 > div", { opacity: 0 });
        gsap.set("#page-hero-title", { filter: "blur(10px)", y: 20 });
        gsap.set("#page-hero-subtitle", { y: 20 });

        gsap.timeline({ delay: 0.2 })
          .to("#page-hero-title", { opacity: 1, filter: "blur(0px)", y: 0, duration: 1.2, ease: "power3.out" })
          .to("#page-hero-subtitle", { opacity: 1, y: 0, duration: 0.8, ease: "power2.out" }, "-=0.8");
        ...
        gsap.set("#page-hero-title, #page-hero-subtitle, #uslugi .grid > *, #faq h2, #faq .space-y-8 > div", { opacity: 1, y: 0, filter: 'none' });
    }
...
function initPortfolioPage() {
    ...
    document.querySelectorAll('.image-gallery-container').forEach(gallery => {
        ...
        if (images.length <= 1) {
            const controls = gallery.querySelectorAll('.gallery-prev, .gallery-next, .gallery-dots');
    ...
    const setFilter = (id) => {
        ...
        btns.forEach(b => {
            ...
            b.classList.remove('bg-surface/60', 'text-primary', 'border-accent-start', 'bg-surface/40', 'text-secondary', 'border-white/10');
...
function initOMniePage() {
    ...
    if (!PREFERS_REDUCED_MOTION) {
        ...
        gsap.set("#page-hero-title, #page-hero-subtitle, .content-box", { opacity: 1, y: 0, filter: 'none' });
    }
}
...
function initNarzedziaPage() {
    ...
    const setActiveTab = (tabToActivate) => {
        ...
        tabs.forEach(tab => {
            ...
            tab.setAttribute('aria-selected', isActive);
            gsap.to(tab, { opacity: isActive ? 1 : 0.7, color: isActive ? '#E2E2E2' : '#A1A1A1', duration: 0.3 });
        });
        panels.forEach(panel => {
            if (panel.id === targetPanelId) {
                ...
                gsap.to(panel, { opacity: 0, duration: 0.3, onComplete: () => gsap.set(panel, { display: 'none' }) });
            }
        });
    };
...
function initKontaktPage() {
    ...
    // --- Animacje wejściowe ---
    if (!PREFERS_REDUCED_MOTION) {
        gsap.set("#page-hero-title, #page-hero-subtitle, #page-hero-email, .content-box", { opacity: 0 });
        gsap.set("#page-hero-title", { filter: "blur(10px)", y: 20 });
        gsap.set("#page-hero-subtitle", { y: 20 });
        ...
        gsap.set("#page-hero-title, #page-hero-subtitle, #page-hero-email, .content-box", { opacity: 1, y: 0, filter: 'none' });
    }
    ...
    function setLoading(state){
      if (!btn) return;
      btn.disabled = state;
      btn.textContent = state ? 'Wysyłanie...' : 'Wyślij wiadomość';
      btn.style.opacity = state ? '0.75' : '1';
    }

    function showStatus(box, message = '') {
        if (!statusBox || !ok || !err) return;
    ...
    if (!form.dataset.listenerAttached) {
        form.addEventListener('submit', async (e) => {
            ...
            if (!data.name || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email) || !data.message || !data.consent) {
                showStatus(err, "Uzupełnij wymagane pola (imię i nazwisko, e‑mail, wiadomość) i zaznacz zgodę.");
                return;
            }
            
            const now = Date.now();
            if (data.ts && now - Number(data.ts) < 3000) {
                showStatus(err, "Wykryto zbyt szybkie wysłanie. Odśwież i spróbuj ponownie.");
                return;
            }
           
            if (data.website) { // Honeypot
                showStatus(ok, "Dziękuję! Wrócę z odpowiedzią możliwie szybko (zwykle tego samego dnia roboczego)."); 
                form.reset();
                return;
            }
            ...
            try {
                ...
                if (!res.ok) throw new Error((await res.json()).message || 'Błąd serwera');
                
                showStatus(ok, "Dziękuję! Wrócę z odpowiedzią możliwie szybko.");
                form.reset();
            } catch (e) {
                showStatus(err, `Coś poszło nie tak. Spróbuj ponownie lub napisz na <a class="underline" href="mailto:${emailAddress}">${emailAddress}</a>.`);
            } finally {
                setLoading(false);
...
function initComingSoonPage() {
    ...
    if (!PREFERS_REDUCED_MOTION) {
        ...
        if (container3D && !container3D.dataset.initialized) {
            ...
            const material = new THREE.MeshStandardMaterial({ color: 0x8B5CF6, emissive: 0x3B82F6, wireframe: true });
...
function initStaticContentPageAnims() {
    ...
    if (!PREFERS_REDUCED_MOTION) {
        ...
        gsap.timeline({ delay: 0.2 })
          .to("#page-hero-title", { opacity: 1, filter: "blur(0px)", y: 0, duration: 1.2, ease: "power3.out" })
          .to("#page-hero-subtitle", { opacity: 1, y: 0, duration: 0.8, ease: "power2.out" }, "-=1.0");
